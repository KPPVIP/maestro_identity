...
RegisterNetEvent('maestro_identity:showRegisterIdentity')
AddEventHandler('maestro_identity:showRegisterIdentity', function()
	if not isDead then
		EnableGui(true)
	end
end)

RegisterNetEvent('maestro_identity:identityCheck')
AddEventHandler('maestro_identity:identityCheck', function(identityCheck)
	hasIdentity = identityCheck
end)

RegisterNetEvent('maestro_identity:saveID')
AddEventHandler('maestro_identity:saveID', function(data)
	myIdentifiers = data
end)
...
RegisterNUICallback('register', function(data, cb)
	...
	if reason == "" then
		TriggerServerEvent('maestro_identity:setIdentity', data, myIdentifiers)
		EnableGui(false)
		Citizen.Wait(500)
